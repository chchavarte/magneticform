...
class FieldPreviewSystem {
  ...
  static Map<String, FieldConfig> calculatePreviewPositions({
    ...
    if (rowAnalysis.isRowFull) {
      ...
      if (autoResizeResult != null) {
        Logger.success('AUTO-RESIZE SUCCESS: Returning auto-resize result');
        return autoResizeResult;
      } else {
        Logger.warning('AUTO-RESIZE FAILED: Trying direct placement');
      }
      ...
      if (availablePosition != null) {
        ...
        Logger.warning(
          'WARNING: Using original width ${(draggedField.width * 100).toInt()}% - NO RESIZE APPLIED',
        );
  ...
  static Map<String, FieldConfig>? _calculateAutoResizePreview({
    ...
    Logger.resize('DEBUG AUTO-RESIZE: Total available space: $totalAvailableSpace');

    if (totalAvailableSpace <= 0) {
      Logger.resize('DEBUG AUTO-RESIZE: No space available');
      return null;
    }
    ...
    if (optimalWidth != null && optimalWidth != draggedField.width) {
      ...
      if (bestPosition != null) {
        Logger.success('RESIZE & POSITION SUCCESS: Creating preview configs');
        final previewConfigs = <String, FieldConfig>{};
    ...
    Logger.resize('DEBUG AUTO-RESIZE: Auto-resize not possible');
    return null;
  }
  ...
  static ({int occupiedColumns, bool isRowFull}) _analyzeRowOccupancy({
    ...
    Logger.overlap('ANALYZING ROW $targetRow OCCUPANCY (excluding $excludeFieldId):');
    ...
    Logger.overlap('  Total occupied: $totalOccupied/6, Is full: $isRowFull');
  ...
  static double? _findBestFitWidth(double availableSpace, double currentWidth) {
    ...
    Logger.resize('DEBUG BEST FIT: Card widths: ${MagneticCardSystem.cardWidths}');
    ...
    for (final width in MagneticCardSystem.cardWidths.reversed) {
      ...
      if (width <= availableSpace && width != currentWidth) {
        Logger.resize('DEBUG BEST FIT: Found best width: $width');
        return width;
      }
    }

    Logger.resize('DEBUG BEST FIT: No suitable width found');
    return null;
  }
    ...
    for (int startCol = 0; startCol <= 6 - columnSpan; startCol++) {
      ...
      for (final entry in currentConfigs.entries) {
        ...
        if (!(testEnd < existingStart || startCol > existingEnd)) {
          hasOverlap = true;
          Logger.overlap('OVERLAP DETECTED: Field ${entry.key} blocks position');
          break;
        } else {
          Logger.overlap('NO OVERLAP: Position is clear');
        }
      }

      if (!hasOverlap) {
        Logger.grid('DEBUG POSITION: Found valid position at column $startCol');
        return testPosition;
      }
    }

    Logger.grid('DEBUG POSITION: No valid position found');
    return null;
  }
